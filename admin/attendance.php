					...
					?>	
						<tr>
							...
							elseif($row['log_type'] ==3){
								...
								$log = "TIME Out AM (IaAS)";
							}elseif($row['log_type'] ==7){
								$log = "TIME IN PM (IaAS)";
							}elseif($row['log_type'] ==8){
								$log = "TIME OUT PM (IaAS)";
							}
							?>
							<td><?php echo $log ?></td>
							<td><?php echo date("h:i a", strtotime($row['datetime_log']))?></td>
							<td><center><button data-id = "<?php echo $row['id']?>" class = "btn btn-sm btn-outline-primary remove_log" type="button"><i class = "fa fa-trash"></i></button></center></td>
						</tr>
					<?php
						}
					?>	
